...
<template>
  <ContainerBox class="h-screen w-screen bg-grimoire-deep text-grimoire-parchment font-sans flex relative overflow-hidden">
    <!-- Cosmic Desktop Background -->
    <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--color-grimoire-ink)_0%,_var(--color-grimoire-deep)_100%)] z-0 pointer-events-none"></div>
    ...
    <div class="relative z-10 flex w-full h-full p-0 sm:p-6 lg:p-10 pointer-events-none">
      ...
      >
        ...
        >
          <div class="absolute top-0 inset-x-0 h-px w-full bg-gradient-to-r from-transparent via-amber-300/30 to-transparent"></div>
          <div class="p-8 flex flex-col h-full z-10 relative">
            <SiteBranding class="mb-12" @reveal="(active: boolean) => revealActive = active" />
            <NavigationMenu :style="fadeOpacity" class="flex flex-col gap-4 mt-8" />
          </div>
          <!-- Ambient Side Glow -->
          <div class="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-grimoire-gold/5 to-transparent pointer-events-none mix-blend-overlay"></div>
        </ContainerBox>

        <!-- Grimoire Display Area (Scrollable) -->
        <ContainerBox :style="fadeOpacity" class="flex-grow h-full overflow-hidden flex flex-col relative">
          <!-- Top subtle gradient for depth -->
          <div class="absolute top-0 left-0 right-0 h-16 bg-gradient-to-b from-grimoire-deep/20 to-transparent z-10 pointer-events-none"></div>
          
          <div ref="scrollContainer" class="flex-grow h-full overflow-y-auto px-8 py-16 sm:px-16 lg:px-24 scroll-smooth custom-scrollbar">
            <ContainerBox tag="main" class="w-full max-w-none mx-auto prose prose-invert prose-grimoire md:prose-lg prose-h1:font-display prose-h2:font-display prose-h3:font-display prose-h1:text-transparent prose-h1:bg-clip-text prose-h1:bg-gradient-to-r prose-h1:from-amber-200 prose-h1:to-grimoire-gold prose-h2:text-grimoire-gold prose-h3:text-grimoire-gold prose-a:text-amber-300 hover:prose-a:text-yellow-200 prose-a:underline-offset-4 prose-strong:text-amber-100 selection:bg-grimoire-gold/40 selection:text-white prose-blockquote:border-l-grimoire-gold prose-blockquote:bg-grimoire-gold/5 prose-blockquote:px-6 prose-blockquote:py-2 prose-blockquote:italic prose-p:leading-relaxed pb-32">
              <slot />
            </ContainerBox>
          </div>
          
          <!-- Bottom subtle gradient -->
          <div class="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-grimoire-deep/20 to-transparent z-10 pointer-events-none"></div>
        </ContainerBox>
